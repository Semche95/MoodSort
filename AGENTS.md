...
- `controllers/`: stateful orchestrators that own a slice of Pixi/DOM state (app, stage, listeners, lifecycle) and coordinate other components, delegating persistence to `services/` and computation to `utils/`. May depend on `types/`, `services/`, `utils/`, `ui/`, and other `controllers/`.
