...
function findCard(controller: TestableController, imageUrl: string): Card {
    const card = controller.cards.find((c: Card): boolean => c.imageUrl === imageUrl)
    if (!card) {
...
describe('CanvasController compact stack action', () => {
    ...
    it('leaves the top card in place and disperses the others around it, without touching unrelated cards or z-order', async () => {
        ...
        const { computeCompactButtonBox } = await import('../utils/stack')
    ...
    it('pushes a single undo/redo entry and does not clear pre-existing history', async () => {
        ...
        const { controller, testable, tick } = await setup(priorEntry)
        const { computeCompactButtonBox } = await import('../utils/stack')
